...
const phraseCatalog: Record<string, string[]> = {
  klasyczne: ["Skok do basenu", "Robot kuchenny", "Latanie samolotem"],
  "filmy-i-seriale": ["Titanic", "Stranger Things", "Matrix"],
  muzyka: ["Gitara elektryczna", "Koncert rockowy", "DJ na weselu"],
  sport: ["Rzut karny", "Bieg maratonski", "Skok o tyczce"],
  zawody: ["Dentysta", "Stolarz", "Ratownik"],
  jedzenie: ["Pizza hawajska", "Lody waniliowe", "Spaghetti"],
  miejsca: ["Lotnisko", "Biblioteka", "Silownia"],
  zwierzeta: ["Pingwin", "Sowa", "Zyrafa"],
  przyslowia: [
    "Co nagle to po diable",
    "Lepszy wrobel w garsci",
    "Jak sobie poscielesz",
  ],
  dev: ["Merge conflict", "Code review", "Deploy w piatek"],
};
...
function getPhraseForTurn(
  ...
  const category =
    ...
    fallbackCategory;
  const phrases = phraseCatalog[category.id] ?? ["Kalambury"];
  const phraseIndex =
